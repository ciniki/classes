//
// The app to add/edit classes class images
//
function ciniki_classes_classimages() {
    this.webFlags = {
        '1':{'name':'Hidden'},
        };
    this.init = function() {
        //
        // The panel to display the edit form
        //
        this.edit = new M.panel('Edit Image',
            'ciniki_classes_classimages', 'edit',
            'mc', 'medium', 'sectioned', 'ciniki.classes.images.edit');
        this.edit.default_data = {};
        this.edit.data = {};
        this.edit.class_id = 0;
        this.edit.sections = {
            '_image':{'label':'Photo', 'type':'imageform', 'fields':{
                'image_id':{'label':'', 'type':'image_id', 'hidelabel':'yes', 'controls':'all', 'history':'no'},
            }},
            'info':{'label':'Information', 'type':'simpleform', 'fields':{
                'name':{'label':'Title', 'type':'text'},
                'webflags':{'label':'Website', 'type':'flags', 'join':'yes', 'flags':this.webFlags},
            }},
            '_description':{'label':'Description', 'type':'simpleform', 'fields':{
                'description':{'label':'', 'type':'textarea', 'size':'small', 'hidelabel':'yes'},
            }},
            '_save':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_classes_classimages.saveImage();'},
                'delete':{'label':'Delete', 'fn':'M.ciniki_classes_classimages.deleteImage();'},
            }},
        };
        this.edit.fieldValue = function(s, i, d) { 
            if( this.data[i] != null ) {
                return this.data[i]; 
            } 
            return ''; 
        };
        this.edit.fieldHistoryArgs = function(s, i) {
            return {'method':'ciniki.classes.classImageHistory', 'args':{'tnid':M.curTenantID, 
                'class_image_id':this.class_image_id, 'field':i}};
        };
        this.edit.addDropImage = function(iid) {
            M.ciniki_classes_classimages.edit.setFieldValue('image_id', iid, null, null);
            return true;
        };
        this.edit.addButton('save', 'Save', 'M.ciniki_classes_classimages.saveImage();');
        this.edit.addClose('Cancel');
    };

    this.start = function(cb, appPrefix, aG) {
        args = {};
        if( aG != null ) { args = eval(aG); }

        //
        // Create container
        //
        var appContainer = M.createContainer(appPrefix, 'ciniki_classes_classimages', 'yes');
        if( appContainer == null ) {
            M.alert('App Error');
            return false;
        }

        if( args.class_id != null && args.class_image_id != null && args.class_image_id == 0 ) {
            this.showEdit(cb, 0, args.class_id);
        } else if( args.class_image_id != null && args.class_image_id > 0 ) {
            this.showEdit(cb, args.class_image_id);
        }
        return false;
    }

    this.showEdit = function(cb, iid, cid) {
        if( iid != null ) { this.edit.class_image_id = iid; }
        if( cid != null ) { this.edit.class_id = cid; }
        if( this.edit.class_image_id > 0 ) {
            var rsp = M.api.getJSONCb('ciniki.classes.classImageGet', 
                {'tnid':M.curTenantID, 'class_image_id':this.edit.class_image_id}, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
                    var p = M.ciniki_classes_classimages.edit;
                    p.data = rsp.image;
                    p.refresh();
                    p.show(cb);
                });
        } else {
            this.edit.reset();
            this.edit.data = {};
            this.edit.refresh();
            this.edit.show(cb);
        }
    };

    this.saveImage = function() {
        if( this.edit.class_image_id > 0 ) {
            var c = this.edit.serializeFormData('no');
            if( c != '' ) {
                M.api.postJSONFormData('ciniki.classes.classImageUpdate', 
                    {'tnid':M.curTenantID, 
                    'class_image_id':this.edit.class_image_id}, c,
                        function(rsp) {
                            if( rsp.stat != 'ok' ) {
                                M.api.err(rsp);
                                return false;
                            } else {
                                M.ciniki_classes_classimages.edit.close();
                            }
                        });
            } else {
                this.edit.close();
            }
        } else {
            var c = this.edit.serializeFormData('yes');
            M.api.postJSONFormData('ciniki.classes.classImageAdd', 
                {'tnid':M.curTenantID, 'class_id':this.edit.class_id}, c,
                    function(rsp) {
                        if( rsp.stat != 'ok' ) {
                            M.api.err(rsp);
                            return false;
                        } else {
                            M.ciniki_classes_classimages.edit.close();
                        }
                    });
        }
    };

    this.deleteImage = function() {
        M.confirm('Are you sure you want to delete this image?',null,function() {
            M.api.getJSONCb('ciniki.classes.classImageDelete', {'tnid':M.curTenantID, 
                'class_image_id':M.ciniki_classes_classimages.edit.class_image_id}, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
                    M.ciniki_classes_classimages.edit.close();
                });
        });
    };
}
