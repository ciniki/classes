...
// Binary file.
//
function ciniki_classes_classFileDownload($ciniki) {
    //  
    ...
    $rc = ciniki_classes_checkAccess($ciniki, $args['business_id'], 'ciniki.classes.classFileDownload'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $strsql = "SELECT ciniki_class_files.id, "
        . "ciniki_class_files.name, ciniki_class_files.extension, "
        . "ciniki_class_files.binary_content "
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.classes.12', 'msg'=>'Unable to find file'));
    }
    ...
    if( $rc['file']['extension'] == 'pdf' ) {
        ...
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.classes.13', 'msg'=>'Unsupported file type'));
    }
    // Specify Filename
