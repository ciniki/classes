...
// Description
// -----------
// This method will turn the classes settings for a business.
//
// Arguments
// ---------
// api_key:
...
function ciniki_classes_settingsGet($ciniki) {
	...
	if( isset($args['page']) && $args['page'] != '' ) {
		$rc = ciniki_core_dbDetailsQueryDash($ciniki, 'ciniki_class_settings', 'business_id', 
			$args['business_id'], 'ciniki.classes', 'settings', 'classes-' . $args['page']);
	} else {
		$rc = ciniki_core_dbDetailsQueryDash($ciniki, 'ciniki_class_settings', 'business_id', 
			$args['business_id'], 'ciniki.classes', 'settings', 'classes');
	}
	if( $rc['stat'] != 'ok' ) {
		return $rc;
	}
